            ...
            <div class="envelope-pocket" :class="{ 'env-fade-out': isFullyCentered }">
                ...
                <div class="env-part env-front" @click="handleOpen">
                    <div class="red-box-main" :class="{ 'mini': isOpened }">
                        <div class="calligraphy">晓滢亲启</div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="isOpened" class="letter-paper"
                ...
                <div class="paper-bg-img" v-if="images.length">
                    <transition name="bg-slideshow">
                        ...
                        <div :key="currentImgIndex" class="img-fill"
                            :style="{ backgroundImage: `url(${images[currentImgIndex]})` }"></div>
                    </transition>
                </div>
            </div>
        </div>
...
const getParaStyle = (align?: string): Record<string, string> => {
    ...
    else if (align === 'top') ta = 'left'; // 映射为物理顶部
...
const handleOpen = async (): Promise<void> => {
    ...
    // 阶梯式动画执行序列
    setTimeout(() => { isLetterUp.value = true; }, 50);          // 1. 信纸微升
    setTimeout(() => { isFullyCentered.value = true; }, 1200);   // 2. 居中并放大，信封消失
    setTimeout(() => {
